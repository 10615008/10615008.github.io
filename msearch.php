...
<?php
				...
				$sql = "select * from tbl_product where pd_name LIKE '".$keyword."%'";
				//echo $sql."<br>";
				$us = mysql_query($sql, $con);
				$rows_num = mysql_num_rows($us);
				
				$sql2 = "select * from tbl_product where pd_price LIKE '".$keyword."%'";
				//echo $sql."<br>";
				$us2 = mysql_query($sql2, $con);
				$rows_num2 = mysql_num_rows($us2);
				
				if ($rows_num == 0 && $rows_num2 == 0)
				{
					echo "沒找到相關資料";
					exit(1);
				}
...
<table style="border: 5px double rgb(109, 2, 107); background-color: rgb(255, 255, 255); width: 800px;" align="center" cellpadding="5" cellspacing="5" frame="border" rules="all">
   <tbody>
     <tr>
       <td>房間</td>
       <td>敘述</td>
...
<?php
				...
				while($data=mysql_fetch_row($us))
				{
					...
					$sql1 = "select * from tbl_category where cat_id='$sid'";
					//echo $sql."<br>";
					$us1 = mysql_query($sql1, $con);
					$data1=mysql_fetch_row($us1);
					
					$cid = $data1[1];
					
				    echo "<tr><td><a href=\"store.php?c=$cid&p=$id\">$data[2]</a></td>";
				    echo "<td>$data[3]</td>";
				...
				while($data2=mysql_fetch_row($us2))
				{
					$id = $data2[0];
					$sid = $data2[1];
				    ...
				    echo "<tr><td><a href=\"store.php?c=$cid&p=$id\">$data2[2]</a></td>";
				    echo "<td>$data2[3]</td>";
					echo "<td>$data[4]</td></tr>";
				    $flag=1;
